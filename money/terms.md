...
> 27. 자기자본비율
- 전체 자산(자본+부채)에서 자본의 비율

- $\dfrac{\text{자기자본}}{\text{총 자본}} * 100$

> 28. 부채비율
- 내 돈(자본)과 타인의 돈(부채)을 비교하는 비율

- $\dfrac{\text{부채(타인자본)}}{\text{자본(자기자본)}} * 100$

> ETF
- 지수를 추종하는 펀드 상품

> 선물거래 
- 투자자가 미래의 특정 시점에 자산을 정해진 가격으로 사고팔기로 약속하는 거래예요. 말 그대로 "미래(Future)"의 거래를 현재에 약속하는 것
- 목적
    + 투기: 가격이 오르거나 내릴 걸 예상해서 수익을 노림
    + 헤지(위험 회피): 기업이 원자재 가격 변동에 대비하기 위해 사용

> 옵션 : 콜옵션, 풋옵션
- 옵션(option)은 특정 자산을 미래에 '정해진 가격'으로 사고팔 수 있는 권리
- 선물과 달리 "권리"만 있고, 의무는 없음이 핵심
- 콜옵션
    + 특정 자산을 미래의 정해진 가격(행사가격)으로 '살 수 있는 권리'
    + 예:
        + 내가 3개월 후에 삼성전자 주식을 6만 원에 살 수 있는 콜옵션을 샀다고 해요.
        + 3개월 뒤 시장 가격이 7만 원이면? → 6만 원에 사서 이득
        + 5만 원이면? → 권리만 있고, 안 사도 됨 (손실은 옵션 가격만)
- 풋옵션
    + 특정 자산을 미래의 정해진 가격으로 '팔 수 있는 권리'
    + 예:
        - 3개월 뒤에 삼성전자 주식을 6만 원에 팔 수 있는 풋옵션을 샀어요.
        - 3개월 뒤 시장 가격이 5만 원이면? → 6만 원에 팔아서 이득
        - 7만 원이면? → 안 팔아도 됨 (손실은 옵션 가격만)

<details>
<summary>선물거래와 옵션거래 차이점</summary>

| 구분        | 선물거래 (Futures)              | 옵션거래 (Options)             |
| --------- | --------------------------- | -------------------------- |
| **계약의무**  | **매수자와 매도자 모두** 계약 이행 의무 있음 | **권리만 있음**, 이행할지 말지는 선택 가능 |
| **손익구조**  | **무한대 손익 가능**, 리스크 큼        | **옵션료(프리미엄)만 손실**, 리스크 제한  |
| **레버리지**  | 높음 (증거금만으로 거래 가능)           | 낮거나 옵션 가격만큼의 비용 부담         |
| **사용 목적** | 투기, 헤지                      | 투기, 헤지, 위험 제한              |
| **거래방식**  | 주로 직접 매도/매수 계약 체결           | **권리 매수/권리 매도**로 거래        |

- 콜옵션  → 오를 것 같을 때 (살 권리)
- 풋옵션  → 떨어질 것 같을 때 (팔 권리)
- 선물거래 → 오르든 떨어지든 무조건 사거나 팔아야 함 (의무)
</details>


> 증거금 
- 거래를 하기 위해 거래소나 증권사에 맡겨야 하는 보증금
- 증거금이 10%라면, 
    - 1000만원 주식 구매를 위해 100만 원만 있으면 거래 가능
    - 2일 후에, 나머지 900만원을 통장에서 차감

> 콜금리
- 은행끼리 대출할 때 적용하는 초단기 이자.

> 양도성예금증서
- 예금은 일반적으로 시간에 따라 돈을 넣는 적금과 달리 초기에 원금을 이미 넣어두고 후에 이자가 적용된 금액을 예금주에게 건내주는 시스템
- 양도성예금증서는 예금주가 정해져 있지 않아 이 예금 통장을 자유롭게 사고 팔 수 있는 증서
- 일반적으로 대출금리(=기준금리 + 가산금리 - 우대금리)를 결정하는데 주요하게 작용

> RP(환매조건부채권)
- 채권을 팔면서 나중에 다시 사겠다고 약속하는 거래
    - A은행이 B투자자에게 국채 100만 원어치를 오늘 팔고, 3일 후 100만 2천 원에 다시 사오겠다고 약속한 것.

> CMA(종합자산관리계좌)
- 투자회사가 만든 예금처럼 쓸 수 있는 계좌
- 예금 + 투자 성격이 섞인 상품
-  CMA vs 예금 vs 통장

    | 구분         | CMA               | 일반 예금통장 | 적금       |
    | ---------- | ----------------- | ------- | -------- |
    | **이자율**    | 보통 예금보다 높음        | 낮음      | 상대적으로 높음 |
    | **입출금 자유** | 가능                | 가능      | 제한적      |
    | **원금 보장**  | ❌ (대부분 RP 등으로 운용) | ✅       | ✅        |
    | **사용 용도**  | 단기 자금 운용          | 일상적인 자금 | 중장기 저축   |

> 양적완화정책
- 중앙은행이 시중에 돈을 더 많이 공급하기 위해 국채 등 자산을 대량으로 매입하는 정책
- ✅ 왜 하는가?
    + 금리를 낮춰도 소비나 투자가 잘 안 되면
    + 중앙은행은 양적완화를 통해 시장에 직접 돈을 뿌려서 유동성을 늘리고 경제를 살리려고 노력
